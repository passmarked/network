...
module.exports = exports = function(payload, fn) {
  ...
  payload.getResponse(function(err, response) {
    ...
    if(S(originalContent || '').isEmpty() === true) {
      ...
      // debug
      payload.warning('whitespace', 'Page content was blank');
    ...
    var minifiedContent = null;

    // try to parse it
    try {

      // try to minify=
      minifiedContent = minify(originalContent, {
    ...
    if(S(minifiedContent || '').isEmpty() === true) {
      ...
      // debugging
      payload.warning('whitespace', 'Content was blank, so skipping')
    ...
    if(delta >= 5 * 1024 && (delta / originalLen) > 0.05) {
